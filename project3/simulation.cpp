...
bool fileValidation(string s){
    //find whether the file can be open
    fstream input;
    input.open(s);
    if (!input){
        input.close();
...
bool isEmpty(creature_t &creature, grid_t &grid){
    //find whether the front is empty & valid
...
bool initWorld(world_t &world, const string &speciesFile,const string &creaturesFile){
    ...
    while ((getline(speciesSummary,creatureming))){
        ...
        fstream creature;
        string filedir;
        filedir.append(directory).append("/").append(creatureming); //append the directory
        if (!fileValidation(filedir)){
            ...
            while (getline(creature,option) && option[0]!='\n' && option[0]!='\r' && option[0]!='\0'){
                    ...
                    bool opright = false;//find whether the instruction is valid
                    if (temp == "hop") {
    ...
    while (getline(worldFile,creatureming)){
        ...
        string temp,direct;
        int row,colum;
        creatu>>temp>>direct>>row>>colum; //find whether the creature is valid
        ...
        if (!flag){
            ...
            return false;}
        if (world.grid.squares[row][colum]!= nullptr){ //find whether overlap
        ...
        world.creatures[size].location.c=colum; //initiate the creature
        for (int i = 1; i <= (int)world.numSpecies; ++i) {
            if (temp==world.species[i].name){
                world.creatures[size].species=&world.species[i];}}
        world.grid.squares[row][colum]=&world.creatures[size]; //make the certain place of grid point the certain creature
    }
    return true;
}
...
void simulateCreature(creature_t &creature, grid_t &grid, bool verbose){
    ...
    while (!Finished) {
        ...
        if (caozuo.op == IFEMPTY) {//do ifempty
            if (isEmpty(creature, grid)) {
                point_t next;
                next = adjacentPoint(creature.location, creature.direction);
                if (grid.squares[next.r][next.c] == nullptr) {
        ...
        if (caozuo.op == IFENEMY) {//do ifenemy
            if (isEmpty(creature, grid)) {
                point_t next;
                next = adjacentPoint(creature.location, creature.direction);
                if (grid.squares[next.r][next.c] != nullptr) {
